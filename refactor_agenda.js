...
let code = fs.readFileSync('app/agenda.tsx', 'utf8');
...
// Add hooks
code = code.replace(/export default function AgendaScreen\(\) \{/, `export default function AgendaScreen() {
...
code = code.replace(/container: \{\s+flex: 1,\s+backgroundColor: ['"]#(?:ffffff|fff|fcfdfe)['"]\s*\}/, "container: {\n    flex: 1,\n    backgroundColor: colors.background\n  }");
...
code = code.replace(/'#c8956c'/g, 'colors.primary');
...
code = code.replace(/color="#c8956c"/g, 'color={colors.primary}');
...
fs.writeFileSync('app/agenda.tsx', code);
console.log('Agenda refactored!');
